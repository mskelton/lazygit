...
type NonModelItem struct {
	...
	// Whatever data is needed to render the item. In many cases a simple string
	// will suffice
	ClientData any
}
...
type ListRenderer struct {
	...
	// Function to render a non-model item. The column positions of the display
	// strings are passed in so that it's possible to align section headers with
	// columns. Only used if getNonModelItems is provided, can be left nil
	// otherwise.
	renderNonModelItem func(item *NonModelItem, columnPositions []int) string
...
func (self *ListRenderer) insertNonModelItems(
	...
	for _, item := range nonModelItems {
		...
		if item.Index+offset >= startIdx {
			lines = slices.Insert(lines, item.Index+offset-startIdx, self.renderNonModelItem(item, columnPositions))
		}
		offset++
	}
	return lines
