...
func TestListRenderer_renderLines(t *testing.T) {
	...
	for _, s := range scenarios {
		t.Run(s.name, func(t *testing.T) {
			...
			var renderNonModelItem func(item *NonModelItem, columnPositions []int) string
			if s.nonModelIndices != nil {
				getNonModelItems = func() []*NonModelItem {
					return lo.Map(s.nonModelIndices, func(modelIndex int, nonModelIndex int) *NonModelItem {
						return &NonModelItem{Index: modelIndex, ClientData: fmt.Sprintf("--- %d (%d) ---", modelIndex, nonModelIndex)}
					})
				}
				renderNonModelItem = func(item *NonModelItem, columnPositions []int) string {
					return item.ClientData.(string)
				}
			}
			self := &ListRenderer{
				list: viewModel,
				...
				renderNonModelItem: renderNonModelItem,
			}
			...
			if s.nonModelIndices != nil {
				getNonModelItems = func() []*NonModelItem {
					return lo.Map(s.nonModelIndices, func(modelIndex int, _ int) *NonModelItem {
						return &NonModelItem{Index: modelIndex}
					})
				}
			}
			self := &ListRenderer{
				...
				getNonModelItems:   getNonModelItems,
				renderNonModelItem: func(_ *NonModelItem, _ []int) string { return "" },
			}
